...
levels_exp <- c("DMSO_A","DMSO_B" ,
                "TSA_A","TSA_B",
                "TSA24hREC_A","TSA24hREC_B"
)
...
if(!file.exists(outFile))
{
    ...
    p <- ggplot(data, aes(x=factor(sample), y=CS, fill=factor(sample))) +
             ...
             #annotate(geom = "rect", ymin = as.numeric(quartiles[c(3),]$first), ymax = as.numeric(quartiles[c(3),]$third), xmin = -Inf, xmax = Inf, fill = rgb(1,0,0,0.5), color=NA) +
             #annotate(geom = "rect", ymin = as.numeric(quartiles[c(4),]$first), ymax = as.numeric(quartiles[c(4),]$third), xmin = -Inf, xmax = Inf, fill = rgb(0,0,1,0.5), color=NA) +	     
             #annotate(geom = "rect", ymin = as.numeric(quartiles[c(5),]$first), ymax = as.numeric(quartiles[c(5),]$third), xmin = -Inf, xmax = Inf, fill = rgb(1,0,0,0.5), color=NA) +
             #annotate(geom = "rect", ymin = as.numeric(quartiles[c(6),]$first), ymax = as.numeric(quartiles[c(6),]$third), xmin = -Inf, xmax = Inf, fill = rgb(0,0,1,0.5), color=NA) +
